@extends('layouts.app', ['page_title' => 'إضافة غرفة دردشة'])
@section('content')
                            ...
                            <div class="col-12 pt-3">
                                <select type="text" name="owner" required class="form-control">
                                    @foreach (App\Models\User::latest()->get() as $user)
                                        <option {{ old('owner') == $user->id ? 'selected' : '' }}
                        ...
                        <div class="col-12 col-lg-6 p-2">
                            ...
                            <div class="col-12 pt-3">
                                <select type="text" name="status" required class="form-control">
                                    <option {{ old('status') == 0 ? 'selected' : '' }} value="0">قيد المراجعه</option>
                                    <option {{ old('status') == 1 ? 'selected' : '' }} value="1">تفعيل</option>
                                    <option {{ old('status') == 2 ? 'selected' : '' }} value="2">ايقاف</option>
                                </select>
                            </div>
                        </div>
                    </div>
